...
import { stripeRefund } from "./api/payments";
...
export const useUpdateBlogs = (id: string) =>
  useMutation(
    (payload: any): Promise<CreateBlogTypes> => updateCoupon(id, payload)
  );
...
export const useStripeRefund = () => {
  return useMutation((payload: StripeRefundTypes) => stripeRefund(payload));
};

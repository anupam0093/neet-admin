          ...
          return (
            <div key={index} className={css["input-card"]}>
              <div className={css["picker"]}>
                ...
                {data ? (
                  <React.Fragment>
                    <img src={handleImageURL(data)} alt="" />
                    <div className={css["controls"]}>
                      <label htmlFor={imagePickerId}>
                        <span className={css["delete-icon"]}>
                          <EditIcon height={18} width={18} color="#222" />
                        </span>
                      </label>
                      <span
                        className={css["delete-icon"]}
                        onClick={() => removeFields(index)}>
                        <DeleteIcon height={16} width={16} color="#222" />
                      </span>
                    </div>
                  </React.Fragment>
                ) : (
                  <label className={css["add-card"]} htmlFor={imagePickerId}>
                    <AddImage height={24} width={24} color="#222" />
                    <p>Pick Image</p>
                  </label>
                )}
              </div>
